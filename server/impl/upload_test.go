...
func TestValidFiles(t *testing.T) {
	...
	err := postUpload(deviceId, name, t)
	if err != nil {
		t.Fatal(err)
	}
}

func TestInvalidDeviceNames_Uploaded(t *testing.T) {
	name := "image.jpeg"
	deviceId := ")**??/\\\\//<>***_+"
	err := postUpload(deviceId, name, t)
	if err != nil {
...
func fakeFileBytes(filename string) []byte {
	ext := filepath.Ext(filename)
	result := ""
	switch ext {
	case ".jpeg":
		result = "\xff\xd8\xff\xe0\x00\x10JFIF\x00\x01\x01\x00\x00\x01\x00\x01\x00\x00\xff\xe1!\xf6Exif\x00\x00II*\x00\b\x00\x00\x00\r\x00\x00\x01\x03\x00\x01\x00\x00\x00\xc0\x0f\x00\x00\x01\x01\x03\x00\x01\x00\x00\x00\xd0\v\x00\x00"
		break
	case ".mp4":
